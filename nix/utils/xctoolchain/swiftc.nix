...
pkgs.runCommand "mk-xctoolchain-swiftc" { } ''
  mkdir -p $out/usr/bin $out/usr/lib/swift $out/lib/swift $out/nix-support
  ...
  ln -s ${pkgs.darwin.xcode}/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/lib/swift/macosx $out/lib/swift/macosx
  mkdir -p $out/bin
